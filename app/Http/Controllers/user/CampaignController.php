...
class CampaignController extends Controller
{
    ...
    public function uploadReportsObd(){
        $soundType      =   CommonClass::getSoundsType();
        $soundList      =   Sound::select('id', 'name')->where('user_id',Auth::user()->id)->where('status',1)->get();
        return view('user.campaign.upload', compact('soundType','soundList'));
    }

    public function storeReportsObd(Request $request){

        //dd($request);

        $authUserId             =   Auth::user()->id;
        $capaignRefIDByApi      =   $request->campaign_ref_id;
        ...
        if ($request->hasFile('excel_file_upload')) {
            ...
            $campaign->camp_type       =    'transactional';
            $campaign->voiceclip       =    $request->voiceclip;
            $campaign->retry_attempt   =    '0';
            $campaign->retry_duration  =    '0';
            $campaign->schedule        =    '';
            $campaign->dtmf_response   =    '';
            $campaign->voice_credit    =    $needChargePerRecord;
            $campaign->whtsapp_response=    '';
            ...
            while (($filedata = fgetcsv($file, 1000, ",")) !== FALSE) {
                ...
                if ($i == 0) {
                    ...
                    $phoneNumber = $filedata[2]; 
                    $status =   0;


                    if($filedata[3] == 'ANSWER'){
                        $status =   1;
                        $call_remarks   =   'Connected';
                        $call_duration  =   $filedata[8];

                    }else{
                        $call_remarks   = $filedata[3];  
                        $call_duration  =   '';
                    }
                    ...
                    if(preg_match($mobileregex, $phoneNumber) === 1 || preg_match($mobileregex91, $phoneNumber) === 1){
                            $data1 = [
                                ...
                                'status'                    =>  $status,
                                'data_type'                 =>  1,
                                'dtmf_response'             =>  $filedata[4],
                                'whtsapp_response'          =>  '0',
                                'call_start_ts'             => $filedata[5],
                                'call_connect_ts'           => $filedata[6],
                                'call_end_ts'               => $filedata[7],
    ...
    public function show(Request $request, $id)
    {
        ...
        return view('user.campaign.view', compact('campaigndata', 'paginate', 'search', 'sort_f', 'sort_by','campaignDate'));
    }
    ...
    { 
        ...
        $campaigndata  = Campaigndata::where('campaign_id',$id)->count();
        if($campaigndata > 0){
            if(Auth::user()->number_permission == 1){
                $campaigndata = Campaigndata::select( 
                    'cli', 
                    'mobile_no', 
                    \DB::raw("CASE WHEN call_remarks IS NULL THEN 'User Not responding' ELSE call_remarks END AS call_remarks"), 
                ...
                $campaigndata = Campaigndata::select( 
                    'cli', 
                    \DB::raw("CONCAT(SUBSTRING(mobile_no, 1, 3), '****', SUBSTRING(mobile_no, -3)) AS formatted_mobile_no"), 
                    \DB::raw("CASE WHEN call_remarks IS NULL THEN 'User Not responding' ELSE call_remarks END AS call_remarks"), 
                    \DB::raw('CASE WHEN dtmf_response = 0 THEN "" ELSE dtmf_response END AS dtmf_response'), 
                    'call_duration'
                )
            ...
            return Excel::download(new CampaignExportClass($campaigndata->take(225000)), 'report.xlsx');
        }else{
             if(Auth::user()->number_permission == 1){
                $campaigndata = Campaignoldata::select( 
                    'cli', 
                    'mobile_no', 
                    \DB::raw("CASE WHEN call_remarks IS NULL THEN 'User Not responding' ELSE call_remarks END AS call_remarks"), 
                    \DB::raw('CASE WHEN dtmf_response = 0 THEN "" ELSE dtmf_response END AS dtmf_response'), 
    ...
    { 
        ...
        $campaigndata  = Campaigndata::where('campaign_id',$id)->get();
    ...
    public function createCampaignWithName(){
        $soundType      =   CommonClass::getSoundsType();
        $soundList      =   Sound::select('id', 'name')->where('user_id',Auth::user()->id)->where('status',1)->get();
        return view('user.campaign.createtts', compact('soundType','soundList'));
    }

    public function storeCampaignWithName(Request $request){
        $this->validate($request, [
                    ...
                    if($request->upload_type == 'csv'){
                        ...
                        if ($request->hasFile('excel_file_upload')) {
                            ...
                            if(($userCredit < $totalChargePerCsv) && ($planId == 0)){
                                ...
                                ///////////////////Create Campaign
                                $createOBDCampaign  =   CommonClass::createOBDCampaignTTS($campaignName, $request->service_no, $campSchDate, $campStartTime, $campEndTime, $retry_duration, $retry_Count, $dtmf_Req, $dtmf_length, $idToken);
                                ...
                                $voiceUpload = CommonClass::campaignVoiceUploadTTS($fullSoundFilePath, $idToken, $capaignNameByApi,$idToken); 
                                ...
                                echo $baseUploadResponse;
                               //=dd($createOBDCampaign,$voiceUpload,$baseUploadResponse);
                                if ($baseUploadResponse === false) {
    ...
    public function campaignMonthData(){
        $soundType      =   CommonClass::getSoundsType();
        $soundList      =   Sound::select('id', 'name')->where('user_id',Auth::user()->id)->where('status',1)->get();
        return view('user.campaign.createmonth', compact('soundType','soundList'));
    }
