								...
								<div class="row">
								    <div class="col-sm-12">
										...
										<div style="float: right;">
											<label>Search:<input type="search" value="{{$search}}" name="search" class="form-control input-sm" placeholder=""></label>
										</div>
									</div>
								</div>
							</form>
							...
							<table
								...
								<tbody>
									...
									<tr>
										...
										<td class="product-headline text-left wide-column">
											@if($r->status == 0)
											<a class="btn btn-primary" onclick="ConfirmDialog('all',{{$r->id}})">Resend {{$r->id}}</a>
											@endif
										</td>
...
<script>
	...
	function downloadFile(campaignId) {
	    ...
	    $.ajax({
	        url: "{{ url('admin/campaign/downloadFile') }}/"+campaignId,
	        method: 'GET',
	...
	function ConfirmDialog(type,id) {
		...
		buttons: {
			Yes: function(sucesss) {
				$(this).remove();
				$.ajax({
				url: "{{ url('admin/campaign/resend') }}/"+id,
				method: 'GET',
				success: function(response) {
					if(response == 1){
