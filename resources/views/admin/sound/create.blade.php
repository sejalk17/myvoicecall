	...
	@endif
		...
		<div class="row">
			<div class="col-xs-12 col-md-12 col-lg-12 col-sm-12">
				<div class="card">
					<div class="card-body">
						<form method="POST" action="{{ route('sound.store') }}" enctype="multipart/form-data">
							@csrf
							<div class="row">
								<div class="col-md-6 form-group">
									<label>File Name</label>
									<input class="form-control" type="text" id="camp_end_datetime" name="name" required value="{{ old('name') }}" placeholder="Enter File name">
								...
								<div class="col-md-6 form-group">
                                    ...
                                    @foreach ($apiProvider as $key => $value)
                                        <option @if(old('apiProvider') == $key) selected @endif value="{{$key}}">{{ $value }}</option>
                                    @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 form-group">
									...
									<select name="ukey" id="ukey" class="form-select">
										...
										@foreach($ukey as $r)
										<option value="{{$r->id}}">{{$r->provider}} / {{$r->cli}} / {{$r->username}}</option>
										@endforeach
									</select>
